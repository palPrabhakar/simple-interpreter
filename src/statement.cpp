...
std::vector<Instruction> StatementAST::GenerateCode(uint &ridx) {
    ...
    if (val) {
        ...
        operations = m_expr->GenerateCode(ridx);
        operations.emplace_back(
            InsCode::store, static_cast<int>(m_expr->GetValue()), m_varName);
    }
...
std::vector<Instruction> IfStatementAST::GenerateCode(uint &ridx) {
    ...
    operations.emplace_back(InsCode::cjmp,
                            static_cast<int>(m_cexpr->GetValue()),
...
std::vector<Instruction> WhileStatementAST::GenerateCode(uint &ridx) {
    ...
    operations.emplace_back(InsCode::cjmp,
                            static_cast<int>(m_cexpr->GetValue()), 2);
