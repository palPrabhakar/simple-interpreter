...
namespace tdb {
    std::vector<std::string> ParseCreateQuery(std::vector<std::pair<Token, std::string>> &tokens, std::size_t index) {
        ...
        if(csm.EOP()) {
            ...
            for(size_t i = 0; i < csm.col_types.size(); ++i) {
                std::cout<<"Col Name: "<<csm.col_names[i]<<", Col Type: "<<csm.col_types[i]<<std::endl;
            }
        } else {
